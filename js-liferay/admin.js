/* ==============================================================
 * Hub Marketing — admin.js (build Liferay)
 * Gerado em: 2026-04-10 17:37:48
 * Contém: config + main.js + admin.js
 * ============================================================== */

// ─── Configuração desta página (ANTES de main.js) ───
window.HUB_PAGE = 'admin';
window.HUB_PAGES = {
  home:        '/web/mkt/home',
  mesas:       '/web/mkt/mesas',
...
(function() {
  ...
  async function loadUsuarios() {
    ...
    var html = '<div class="admin-panel-header">' +
      ...
      '<div class="d-flex align-items-center" style="gap:0.75rem;">' +
        ...
        '<button class="btn btn-sm btn-primary btn-icon" onclick="window._adminAddUser()"><i class="fa-solid fa-user-plus"></i> Novo Usuário</button>' +
      '</div>' +
      '</div>';
  ...
  window._adminAddUser = function() {
    var formHtml = '' +
      '<div class="form-group"><label>Usuário <span class="text-danger">*</span></label>' +
        '<input class="form-control" id="nu-user-name" placeholder="nome.sobrenome (sem @dominio)"></div>' +
      '<div class="form-group"><label>Nome completo <span class="text-danger">*</span></label>' +
        '<input class="form-control" id="nu-nome" placeholder="Nome Completo"></div>' +
      '<div class="form-group"><label>Apelido <span class="text-danger">*</span></label>' +
        '<input class="form-control" id="nu-apelido" placeholder="Como prefere ser chamado"></div>' +
      '<div class="form-group"><label>Email <span class="text-danger">*</span></label>' +
        '<input class="form-control" id="nu-email" type="email" placeholder="email@sicoob.com.br"></div>' +
      '<div class="form-group"><label>Telefone</label>' +
        '<input class="form-control" id="nu-telefone" placeholder="(00) 00000-0000"></div>' +
      '<div class="form-group"><label>Aniversario</label>' +
        '<input type="date" class="form-control" id="nu-aniversario"></div>' +
      '<div class="form-group"><label>Endereco</label>' +
        '<input class="form-control" id="nu-endereco" placeholder="Rua, numero, complemento"></div>' +
      '<div class="row">' +
        '<div class="col-8"><div class="form-group"><label>Bairro</label>' +
          '<input class="form-control" id="nu-bairro" placeholder="Bairro"></div></div>' +
        '<div class="col-4"><div class="form-group"><label>CEP</label>' +
          '<input class="form-control" id="nu-cep" placeholder="00000-000"></div></div>' +
      '</div>' +
      '<div class="form-group"><label>Gerencia</label>' +
        '<select class="form-control" id="nu-gerencia">' + gerenciaOptions(null) + '</select></div>' +
      '<div class="form-group"><label>Coordenacao</label>' +
        '<select class="form-control" id="nu-coordenacao">' + coordenacaoOptions(null, null) + '</select></div>' +
      '<div class="form-group"><label>Nucleo</label>' +
        '<select class="form-control" id="nu-nucleo">' + nucleoOptions(null, null) + '</select></div>' +
      '<div class="form-group"><label>Senioridade</label><select class="form-control" id="nu-senioridade">' +
        '<option value="">— Não definida —</option>' +
        ['Gerente','Coordenador','Especialista','Sênior','Pleno','Júnior','Assistente','Estagiário','Jovem Aprendiz'].map(function(s) {
          return '<option value="' + s + '">' + s + '</option>';
        }).join('') +
      '</select></div>' +
      '<div class="form-check mb-2"><input type="checkbox" class="form-check-input" id="nu-terceirizado">' +
        '<label class="form-check-label" for="nu-terceirizado">Terceirizado</label></div>' +
      '<div class="form-check mb-2"><input type="checkbox" class="form-check-input" id="nu-is-admin">' +
        '<label class="form-check-label" for="nu-is-admin">Admin</label></div>' +
      '<div class="form-check mb-2"><input type="checkbox" class="form-check-input" id="nu-is-gestor">' +
        '<label class="form-check-label" for="nu-is-gestor">Gestor</label></div>';

    showEditModal('Novo Usuário', formHtml, async function() {
      var userName = document.getElementById('nu-user-name').value.trim().toLowerCase();
      var nome     = document.getElementById('nu-nome').value.trim();
      var apelido  = document.getElementById('nu-apelido').value.trim();
      var email    = document.getElementById('nu-email').value.trim();
      if (!userName) { hub.utils.showToast('Preencha o campo Usuário', 'warning'); return; }
      if (!nome)     { hub.utils.showToast('Preencha o nome completo', 'warning'); return; }
      if (!apelido)  { hub.utils.showToast('Preencha o apelido', 'warning'); return; }
      if (!email)    { hub.utils.showToast('Preencha o email', 'warning'); return; }

      // Verifica duplicidade de user_name
      var exists = allUsers.some(function(u) { return u.user_name === userName; });
      if (exists) { hub.utils.showToast('Usuário "' + userName + '" já existe', 'error'); return; }
      ...
      var row = {
        ...
        apelido:          apelido,
        email:            email,
        telefone:         document.getElementById('nu-telefone').value.trim() || null,
        aniversario:      document.getElementById('nu-aniversario').value || null,
        endereco:         document.getElementById('nu-endereco').value.trim() || null,
        bairro:           document.getElementById('nu-bairro').value.trim() || null,
        cep:              document.getElementById('nu-cep').value.trim() || null,
        gerencia_id:      document.getElementById('nu-gerencia').value || null,
        coordenacao_id:   document.getElementById('nu-coordenacao').value || null,
        nucleo_id:        document.getElementById('nu-nucleo').value || null,
        senioridade:      document.getElementById('nu-senioridade').value || null,
        terceirizado:     document.getElementById('nu-terceirizado').checked,
        is_admin:         document.getElementById('nu-is-admin').checked,
        is_gestor:        document.getElementById('nu-is-gestor').checked,
        is_active:        true,
        profile_complete: false
      };

      var res = await hub.sb.from('users').insert(row);
      if (res.error) {
        hub.utils.showToast('Erro: ' + res.error.message, 'error');
      } else {
        hub.utils.showToast('Usuário criado com sucesso', 'success');
        hideEditModal();
        loadUsuarios();
      }
    });
    ...
    // Cascading dropdowns
    document.getElementById('nu-gerencia').addEventListener('change', function() {
      document.getElementById('nu-coordenacao').innerHTML = coordenacaoOptions(this.value, null);
      document.getElementById('nu-nucleo').innerHTML = nucleoOptions(null, null);
    });
    document.getElementById('nu-coordenacao').addEventListener('change', function() {
      document.getElementById('nu-nucleo').innerHTML = nucleoOptions(this.value, null);
    });
  };
  ...
  document.addEventListener('hub:ready', function(e) {
    ...
    document.addEventListener('keydown', function(e) {
      ...
      var iconesOv = document.getElementById('icones-modal-overlay');
      if (iconesOv && iconesOv.classList.contains('show')) {
        iconesOv.classList.remove('show');
      } else if (confirmOverlay && confirmOverlay.classList.contains('show')) {
        confirmOverlay.classList.remove('show');
      } else if (editOverlay && editOverlay.classList.contains('show')) {
        hideEditModal();
      }
    });
